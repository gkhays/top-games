...
def read_keys_from_yaml(file_path):
    ...
    return config.get('api_key'), config.get('steam_id')

# Get the list of games and playtime for the specified user
def get_top_games(api_key, steam_id):
    ...
    if response.status_code == 200:
        ...
        sorted_games = sorted(games, key=lambda x: x['playtime_forever'], reverse=True)[:10]
        return sorted_games
    else:
...
def display_top_games(top_games):
    if top_games:
        print("Top 10 Games Based on Hours Played:")
...
if __name__ == "__main__":
    api_key, steam_id = read_keys_from_yaml('config.yaml')
    top_games = get_top_games(api_key, steam_id)
